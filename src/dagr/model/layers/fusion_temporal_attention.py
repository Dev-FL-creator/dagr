...
from dagr.model.networks.snn_backbone_yaml import TemporalTransformerEncoder
...
class SpikeCAFR(nn.Module):
    ...
    def __init__(self, rgb_in_channels: int, evt_in_channels: int, out_channels: int, num_heads: int = 8):
        ...
        # Add temporal transformer encoder for evt aggregation
        self.temporal_aggregator = TemporalTransformerEncoder(
            channels=out_channels, num_heads=num_heads, num_layers=1, dropout=0.0
        )
    ...
    @staticmethod
    def _tbnc_to_bchw(x: torch.Tensor, H: int, W: int, temporal_aggregator=None) -> torch.Tensor:
        # [T,B,N,C] -> [B,N,C] -> [B,C,H,W]
        if temporal_aggregator is not None:
            x = x.permute(1, 3, 0, 2).contiguous()  # [T,B,N,C] -> [B,C,T,N]
            x = temporal_aggregator(x.view(x.size(0), x.size(1), x.size(2), H, W))  # Aggregate over T
            if len(x.shape) == 4:  # Ensure output is [B, C, H, W]
                return x
            elif len(x.shape) == 3:  # Handle case where spatial dimensions are missing
                B, C, N = x.shape
                x = x.permute(0, 2, 1).contiguous().view(B, C, H, W)
        else:
            x = x.mean(dim=0)  # Default mean over T
        
        # Ensure x has the correct shape before unpacking
        if len(x.shape) != 4:
            raise ValueError(f"Expected x to have 4 dimensions after aggregation, but got shape {x.shape}")
    ...
    def forward(self, rgb: torch.Tensor, evt: torch.Tensor) -> torch.Tensor:
        ...
        out_rgb = self._tbnc_to_bchw(out_rgb, H, W, self.temporal_aggregator)
        ...
        out_evt = self._tbnc_to_bchw(out_evt, H, W, self.temporal_aggregator)
