...
class SpikformerV3Extractor(nn.Module):
    def __init__(
        ...
        self.use_trilinear = bool(getattr(args, "sdt_use_trilinear", True))
        self.temporal_pool_mode = str(getattr(args, "sdt_temporal_pool", "attention"))
    ...
    def _events_to_voxel_trilinear(self, data, height, width, T):
        ...
        if torch.rand(1).item() < 0.01:
            print(f"[SDT Trilinear Voxel] Max: {frames.max().item():.4f}, Mean: {frames.mean().item():.6f}", flush=True)
    ...
    @staticmethod
    def _events_to_frames_simple(data, height, width):
        device = data.x.device
        batch_size = int(data.num_graphs) if hasattr(data, "num_graphs") else 1
    ...
    def _prepare_input(self, x):
        if Data is not None and isinstance(x, Data):
            if self.use_trilinear:
                frames = self._events_to_voxel_trilinear(x, self.height, self.width, self.T)
            else:
                frames = self._events_to_frames_simple(x, self.height, self.width)
