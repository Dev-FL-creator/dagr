...
def draw(ax, data, title, cmap, vmin, vmax, fontsize=7, annotation=None):
...
gs = gridspec.GridSpec(6, 2, figure=fig, top=0.94, bottom=0.03,
                       hspace=0.35, wspace=0.45)

fig.text(0.27, 0.968, "Standard Cross-Attention",      fontsize=13, ha="center", weight="bold", color="#b30326")
fig.text(0.73, 0.968, "Spike-Driven Cross-Attention",  fontsize=13, ha="center", weight="bold", color="#1a1a6e")
...
im = draw(ax_q_raw, raw_q[:, :D], "(a) Q activations  [continuous + noise]",
          "RdBu_r", -qk_v, qk_v, annotation=f"range [{raw_q[:,:D].min():.2f}, {raw_q[:,:D].max():.2f}]  noisy")
plt.colorbar(im, ax=ax_q_raw, fraction=0.046, pad=0.04)

im = draw(ax_q_spk, q_spk[:, :D], "(b) Q activations  [spike clamp to (0,1)]",
          "RdBu_r", -qk_v, qk_v, annotation="range [0.0, 1.0]  negative noise removed")
plt.colorbar(im, ax=ax_q_spk, fraction=0.046, pad=0.04)

im = draw(ax_k_raw, raw_k[:, :D], "(c) K activations  [continuous + noise]",
          "RdBu_r", -qk_v, qk_v, annotation=f"range [{raw_k[:,:D].min():.2f}, {raw_k[:,:D].max():.2f}]  noisy")
plt.colorbar(im, ax=ax_k_raw, fraction=0.046, pad=0.04)

im = draw(ax_k_spk, k_spk[:, :D], "(d) K activations  [spike clamp to (0,1)]",
          "RdBu_r", -qk_v, qk_v, annotation="range [0.0, 1.0]  negative noise removed")
plt.colorbar(im, ax=ax_k_spk, fraction=0.046, pad=0.04)

l_v = max(abs(logits_raw).max(), abs(logits_spk).max())
im = draw(ax_l_raw, logits_raw, "(e) Attention logits  [continuous]",
          "coolwarm", -l_v, l_v, annotation=f"std = {logits_raw.std():.2f}  high variance → collapse risk")
plt.colorbar(im, ax=ax_l_raw, fraction=0.046, pad=0.04)

im = draw(ax_l_spk, logits_spk, "(f) Attention logits  [bounded]",
          "coolwarm", -l_v, l_v, annotation=f"std = {logits_spk.std():.2f}  bounded logits → stable attention")
plt.colorbar(im, ax=ax_l_spk, fraction=0.046, pad=0.04)

a_vmax = max(attn_raw.max(), attn_spk.max())
im = draw(ax_a_raw, attn_raw, "(g) Attention weights  [collapse risk]",
          "YlOrRd", 0, a_vmax, annotation=f"entropy={entropy_raw:.2f}  collapse={collapse_raw:.0f}%")
plt.colorbar(im, ax=ax_a_raw, fraction=0.046, pad=0.04)

im = draw(ax_a_spk, attn_spk, "(h) Attention weights  [stable]",
          "YlOrRd", 0, a_vmax, annotation=f"entropy={entropy_spk:.2f}  collapse={collapse_spk:.0f}%")
plt.colorbar(im, ax=ax_a_spk, fraction=0.046, pad=0.04)

def plot_curve(ax, x, m_r, s_r, m_s, s_s, ylabel, title_label):
    ax.plot(x, m_r, color="#b30326", linewidth=2.5)
    ax.fill_between(x, m_r - s_r, m_r + s_r, alpha=0.25, color="#b30326")
    ax.plot(x, m_s, color="#1a1a6e", linewidth=2.5)
    ax.fill_between(x, m_s - s_s, m_s + s_s, alpha=0.25, color="#1a1a6e")
    ax.set_xlabel("Input noise level (std)", fontsize=11)
    ax.set_ylabel(ylabel, fontsize=11)
    ax.set_title(title_label + "\n(mean ± 1 std over 50 runs)", fontsize=11, pad=6)
    ax.grid(axis='y', alpha=0.3)
    ax.set_xlim(0, 2.0)
    legend_elements = [
        Line2D([0], [0], color="#b30326", linewidth=2.5, label="Standard (mean)"),
        Patch(facecolor="#b30326", alpha=0.35, label="Standard ± 1 std"),
        Line2D([0], [0], color="#1a1a6e", linewidth=2.5, label="Spike (mean)"),
        Patch(facecolor="#1a1a6e", alpha=0.35, label="Spike ± 1 std"),
    ]
...
ax_col.axhline(0, color='gray', linewidth=0.8, linestyle='--')
...
plot_curve(ax_lvar,
           ...
           "Logit variance (lower = stable attention distribution)",
           "(k) Attention logit variance vs. noise level")

plt.savefig("attn_compare.png", dpi=150, bbox_inches="tight")
plt.show()