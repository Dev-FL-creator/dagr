...
class DAGR(YOLOX):
    def __init__(self, args, height, width):
        ...
        no_events = hasattr(args, 'no_events') and getattr(args, 'no_events')
        print(f"Debug: no_events: {no_events}")
        ...
        if use_image and no_events:
            ...
            # 设置 image_only_mode 标志，让 forward 只返回 outputs 列表
            backbone.image_only_mode = True
            
            # 从 ImageBackbone 获取输出通道数
            in_channels = backbone.output_channels  # [256, 512]
            strides = [16, 32]  # layer3 和 layer4 的 stride
            
            # 使用本文件中定义的 CNNHead（而不是从 dagr_fusion_seperate_heads_v3 导入）
            # CNNHead 类定义在本文件末尾
            head = CNNHead(
            ...
            # 设置 backbone 属性以兼容后续代码
            backbone.num_classes = getattr(args, "num_classes", 2)
            backbone.strides = strides
            backbone.out_channels = in_channels
            backbone.use_image = True
            backbone.num_scales = len(in_channels)
        
        # 如果 use_snn=True, use_image=True 且 no_events=False，使用双分支模式
        elif use_snn and use_image and HybridBackbone is not None:
        ...
        # 纯 SNN 模式（无 image）
            ...
            print(f"Debug: running with SNN ONLY mode (no image)")
            if use_sdt:
        ...
        # 传统 GNN 模式
            ...
            print(f"Debug: running with traditional GNN mode")
    ...
    def forward(self, x: Data, reset=True, return_targets=True, filtering=True):
        ...
        if self.training:
            # Image only 模式：只使用 bbox0 (图像的 GT)
            if hasattr(self.backbone, 'use_image') and self.backbone.use_image and hasattr(self.backbone, 'image_only_mode') and self.backbone.image_only_mode:
                # 在 image only 模式下，使用 bbox0（图像的 ground truth）
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                output, grid = self.get_output_and_grid(
                    output, k, stride_this_level, x.type()
                )
                x_shifts = grid[:, :, 0]
                y_shifts = grid[:, :, 1]
                expanded_strides = torch.zeros(1, grid.shape[1]).fill_(stride_this_level).type_as(output)
                ...
                if k == 0:
                    ...
                    outputs_dict["outputs"].append(output)
                    outputs_dict["x_shifts"].append(x_shifts)
                    outputs_dict["y_shifts"].append(y_shifts)
                    outputs_dict["expanded_strides"].append(expanded_strides)
            else:
                output = torch.cat(
        ...
        if self.training:
            return self.get_losses(
                imgs,
                outputs_dict["x_shifts"],
                outputs_dict["y_shifts"],
                outputs_dict["expanded_strides"],
                labels,
                torch.cat(outputs_dict["outputs"], 1),
                outputs_dict["origin_preds"],
                dtype=xin[0].dtype,
            )
        else:
...
class GNNHead(YOLOXHead):
    ...
    def forward(self, xin: Data, labels=None, imgs=None):
        ...
        if self.training:
            if self.use_image:
                # 如果 no_events=True，只返回 image 分支的损失
                if self.no_events:
                    return self.get_losses(
                        imgs,
                        image_out['x_shifts'],
