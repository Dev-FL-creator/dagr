...
try:
    from dagr.model.heads.yolov13_adapter import YOLOv13HeadAdapter, HybridHeadV2YOLOv13
except ImportError:
    YOLOv13HeadAdapter = None
    HybridHeadV2YOLOv13 = None
...
class DAGR(YOLOX):
    def __init__(self, args, height, width):
        ...
        if use_snn and getattr(args, 'use_image', False) and HybridBackbone is not None:
            ...
            use_yolov13_head = getattr(args, 'use_yolov13_head', False)

            if use_yolov13_head:
                if HybridHeadV2YOLOv13 is None:
                    raise ImportError("YOLOv13 head requested but dagr.model.heads.yolov13_adapter not available.")
                
                head = HybridHeadV2YOLOv13(
                    num_classes=backbone.num_classes,
                    strides=backbone.strides,
                    in_channels_fused=backbone.out_channels,
                    in_channels_image=in_channels_image,
                    in_channels_mad=None,
                    args=args
                )
                print("[DAGR] Using YOLOv13 head (HybridHeadV2YOLOv13) for 2-branch fusion.")
            else:
                head = HybridHead(
                    num_classes=backbone.num_classes,
                    strides=backbone.strides,
            ...
            if use_yolov13_head:
                if YOLOv13HeadAdapter is None:
                    raise ImportError("YOLOv13 head requested but dagr.model.heads.yolov13_adapter not available.")
                
                head = YOLOv13HeadAdapter(
                    num_classes=backbone.num_classes,
                    strides=backbone.strides,
                    in_channels=backbone.out_channels,
                    args=args
                )
                print("[DAGR] Using YOLOv13 head (YOLOv13HeadAdapter) for single-branch mode.")
            else:
    ...
    def forward(self, x: Data, reset=True, return_targets=True, filtering=True):
        ...
        if self.training:
            ...
            use_yolov13_head = False
            if hasattr(self, 'head'):
                if YOLOv13HeadAdapter is not None and isinstance(self.head, YOLOv13HeadAdapter):
                    use_yolov13_head = True
                elif HybridHeadV2YOLOv13 is not None and isinstance(self.head, HybridHeadV2YOLOv13):
                    use_yolov13_head = True

            if use_yolov13_head:
                if self.backbone.use_image:
                    if isinstance(self.backbone, HybridBackbone):
                        backbone_outputs = self.backbone(x)
                    else:
                        backbone_outputs = self.backbone(x, reset=reset)
                    
                    if isinstance(backbone_outputs, tuple) and len(backbone_outputs) >= 2:
                        fused_feats, image_feats = backbone_outputs[0], backbone_outputs[1]
                    else:
                        fused_feats = backbone_outputs
                        image_feats = None
                    
                    imgs = x.image if hasattr(x, 'image') else None
                    targets0 = convert_to_training_format(x.bbox0, x.bbox0_batch, x.num_graphs)
                    labels = (targets, targets0)
                    
                    loss_dict = self.head((fused_feats, image_feats, None), labels=labels, imgs=imgs)
                    return loss_dict
                else:
                    if isinstance(self.backbone, HybridBackbone):
                        backbone_outputs = self.backbone(x)
                    ...
                    loss_dict = self.head(backbone_outputs, labels=targets, imgs=imgs)
                    return loss_dict
            else:
                if self.backbone.use_image:
                    targets0 = convert_to_training_format(x.bbox0, x.bbox0_batch, x.num_graphs)
        ...
        if use_yolov13_head:
            if self.backbone.use_image:
                ...
                outputs = self.head((fused_feats, image_feats, None), labels=None, imgs=None)
            else:
                if isinstance(self.backbone, HybridBackbone):
                    backbone_outputs = self.backbone(x)
                else:
                    backbone_outputs = self.backbone(x, reset=reset)
                outputs = self.head(backbone_outputs, labels=None, imgs=None)
        else:
