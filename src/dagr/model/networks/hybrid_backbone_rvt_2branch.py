...
class HybridBackbone(nn.Module):
    ...
    def __init__(self, args, height: int, width: int):
        ...
        # Determine which event backbone to use
        backbone_type = str(getattr(args, "backbone_type", "")).lower()
        self.use_sdt_v3 = backbone_type == "sdtv3"
        self.use_rvt = backbone_type == "rvt"
        ...
        if self.use_rvt:
            # Use RVT backbone for event branch
            self.snn = RVTExtractor(
                args, 
                height=height, 
                width=width, 
                pretrained_weight=getattr(args, "load_pretrained_weight", None)
            )
            evt_channels = list(self.snn.out_channels)  # [128, 256, 512] by default
            rgb_fuse_channels = [c3_ch, c4_ch, c5_ch]
            self.strides = [8, 16, 32]
            self.out_channels = rgb_fuse_channels
            print(f"[HybridBackbone] Using RVT backbone. Event channels: {evt_channels}, RGB channels: {rgb_fuse_channels}")
        elif self.use_sdt_v3:
    ...
    def forward(self, data):
        ...
        if self.use_rvt or self.use_sdt_v3:
